...
def get_support_blocks():
    ...
    for block in acad.iter_objects('AcDbBlockReference', dont_cast=True):
        if block.Layer == "SUPPORT_TAGS":
            ...
            # print(block.Handle)
            support_blocks_list.append(block.Handle)
        else:
            continue
    return support_blocks_list
...
def check_load_sd_tags():
    ...
    support_blocks_list = get_support_blocks()
    ...
    for entity in support_blocks_list:
        progress_percentage = f"{round(((progress / len(support_blocks_list)) * 100), 0)} %"
